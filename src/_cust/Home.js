			...
			<Drawer variant="persistent" anchor="left" open={open} classes={{paper: classes.drawerPaper,}}>
				...
				<List>
					...
					<ListItem button>
						...
						<ListItemText primary="Request Instance" onClick={()=>{this.switch_container('instance_req')}}/>
					</ListItem>
					<ListItem button>
						<ListItemIcon> 
				        	<Timeline/> 
				        </ListItemIcon>
						<ListItemText primary="Usage" onClick={()=>{this.switch_container('manage_instances')}}/>
					</ListItem>
					<Divider/>
					<ListItem button>
				...
				<div className={classes.appFrame}>
					...
					<main className={classNames(classes.content, classes[`content-${anchor}`], {
						...
						{(()=>{
							...
							else if(this.state.main_container==='payment')
								return <Payment switch={this.switch_container}/>;
							else //default
